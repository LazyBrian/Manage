...
namespace Lay.Manage.Tests
{
    public abstract class ManageTestBase : AbpIntegratedTestBase<ManageTestModule>
    {
        ...
        protected void UsingDbContext(Action<ManageDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ManageDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ManageDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ManageDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ManageDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ManageDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ManageDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ManageDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ManageDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ManageDbContext, Task<T>> func)
        {
            T result;
